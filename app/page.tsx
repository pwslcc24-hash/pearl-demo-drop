...
// These reps get the celebration + their song when a demo completes, but
// never appear in leaderboard numbers (podium, leaderboard list, or the
// "N demos complete" line in Recent Wins).
const SONG_ONLY_REPS = new Set(["Ava Geertsen", "Amber Washington", "Katelyn Moric"]);
...
const isSongOnly=(name:string)=>SONG_ONLY_REPS.has(name)||[...SONG_ONLY_REPS].some(songOnly=>normalizeRepName(songOnly)===normalizeRepName(name));
const isHiddenFromLeaderboard=(name:string)=>isLeaderboardExcluded(name)||isSongOnly(name);
const mergeMonthlyCounts=(items:{repName:string;count:number}[]|undefined)=>{
  ...
  for(const item of items??[]){
    ...
    if(!displayName||isHiddenFromLeaderboard(displayName))continue;
