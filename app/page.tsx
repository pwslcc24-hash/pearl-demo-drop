...
export default function Home() {
  ...
  const renderPodiumSlot=(entry:LeaderboardEntry,position:1|2|3,compact=false)=>(
    <div key={entry.repName} className={`podium-slot place-${position}${compact?" is-compact":""}`}>{renderFace(entry.repName,PROFILE_PHOTOS,`podium-face place-${position}`)}<div className="podium-copy"><strong>{medalForRank(entry.rank)?<><span className="podium-medal podium-medal-inline" aria-hidden="true">{medalForRank(entry.rank)}</span>{entry.repName}</>:entry.repName}</strong>{compact?<><span>{entry.count} / {entry.quota} completes</span><em>{entry.pct}% to quota</em></>:<div className="podium-leader-stats"><span className="podium-leader-count">{entry.count} / {entry.quota}</span><em className="podium-leader-pct">{entry.pct}%</em><div className="podium-leader-bar quota-bar" aria-hidden="true"><i style={{width:`${entry.pct}%`}}/></div></div>}</div></div>
  );
  const renderPodium=()=>(
