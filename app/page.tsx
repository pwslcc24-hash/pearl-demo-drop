...
export default function Home() {
  ...
  const renderPodium=()=>(
    <div className="podium-stage"><div className="podium">{topThree.length?topThree.map((entry,index)=><div key={entry.repName} className={`podium-row place-${(index+1) as 1|2|3}-row`}>{renderPodiumSlot(entry,(index+1) as 1|2|3)}</div>):<p className="podium-empty">Waiting for monthly counts from HubSpot…</p>}</div></div>
  );
  ...
  return <main className={`app-shell ${celebrating||leaderCelebrating?"is-celebrating":""} ${leaderCelebrating?"is-new-leader":""} ${showDemoTakeover?"is-demo-celebrating":""}`}>
    ...
    <section className="stage"><div className="ambient orb-one"/><div className="ambient orb-two"/><div className="grid-lines"/>{(celebrating||leaderCelebrating)&&<div className="confetti" aria-hidden="true">{Array.from({length:90}).map((_,i)=><i key={i} style={{"--x":`${(i*37)%101}%`,"--mid":`${((i*29)%80)-40}px`,"--drift":`${((i*53)%180)-90}px`,"--delay":`${(i%12)*.035}s`,"--duration":`${2.4+(i%9)*.13}s`,"--spin":`${540+(i%8)*135}deg`,"--w":`${5+(i%4)*2}px`,"--h":`${i%5===0?7:12+(i%4)*3}px`} as CSSProperties}/>)}</div>}
      {leaderCelebrating&&newLeaderName?<div className="leader-takeover" role="status" aria-live="polite"><div className="leader-takeover-glow" aria-hidden="true"/><div className="leader-takeover-content"><span className="leader-takeover-crown" aria-hidden="true">👑</span><p className="leader-takeover-label">NEW LEADER</p>{renderFace(newLeaderName,PROFILE_PHOTOS,"leader-takeover-face")}<h1 className="leader-takeover-name">{newLeaderName}</h1><p className="leader-takeover-stats"><b>{demoCountFor(newLeaderName)} / {quotaForRep(newLeaderName)}</b> demo completes · <b>{pctToQuota(demoCountFor(newLeaderName),quotaForRep(newLeaderName))}%</b> to quota</p></div>{renderPlayerCard("leader-takeover-player","LEADER CELEBRATION")}</div>:showDemoTakeover?renderDemoTakeover():<div className="stage-layout"><p className="stage-month-label">MONTHLY LEADERS · {monthLabel}</p><div className="stage-podium">{renderPodium()}</div><div className="stage-leaderboard">{renderLeaderboard()}</div></div>}
    </section>
    <section className="control-strip"><div className="recent-wins"><span className="strip-label">RECENT WINS</span><div className="win-list">{wins.slice(0,5).map(win=>{const hideCount=hideRecentWinCount(win.repName);const badge=hideCount?null:rankBadgeForRep(leaderboard,win.repName);return <button key={win.id} onClick={()=>launch(win)} className="win-item"><span className="win-faces-col"><span className="win-faces">{PROFILE_PHOTOS[win.repName]?<img className="avatar" src={PROFILE_PHOTOS[win.repName]} alt=""/>:<span className="avatar">{win.repName.split(" ").map(p=>p[0]).slice(0,2).join("")}</span>}{win.aeName?(photoFor(win.aeName,AE_PHOTOS)?<img className="avatar avatar-ae" src={photoFor(win.aeName,AE_PHOTOS)!} alt=""/>:<span className="avatar avatar-ae">{initialsFor(win.aeName)}</span>):null}</span>{badge&&<span className="win-rank-badge">{badge}</span>}</span><span className="win-copy"><strong>{win.repName}</strong>{win.aeName&&<small className="win-ae-line">AE: {win.aeName}</small>}{!hideCount&&<small><b>{demoCountFor(win.repName)}</b> demos complete</small>}<em>{formatCompletedAt(win.createdAt)}</em></span></button>})}</div></div></section>
    {showSetup&&<div className="modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)setShowSetup(false)}}><section className="song-modal" role="dialog" aria-modal="true" aria-labelledby="song-modal-title"><div className="modal-head"><div><span className="strip-label">REP CELEBRATION</span><h2 id="song-modal-title">{setupRep?setupRep:"Set up songs"}</h2></div><button onClick={()=>setShowSetup(false)} aria-label="Close">×</button></div>{!setupRep?<><p>Click a song to test that rep&apos;s demo complete on the live board. Click CHANGE to edit their clip.</p><div className="rep-song-list">{SONG_SDR_NAMES.map(name=>{const saved=songs.find(s=>normalizeRepName(s.repName)===normalizeRepName(name));const songLabel=saved?`${saved.title} · ${saved.artist}`:"Default song";return <div key={name} className="rep-song-item"><span className="rep-list-avatar">{PROFILE_PHOTOS[name]?<img src={PROFILE_PHOTOS[name]} alt=""/>:name.split(" ").map(p=>p[0]).slice(0,2).join("")}</span><div className="rep-song-copy"><button type="button" className="rep-song-name" onClick={()=>chooseSetupRep(name)}><strong>{name}</strong></button><button type="button" className="rep-song-play" onClick={()=>testDemoComplete(name)} title={`Test demo complete for ${name}`}>{songLabel}</button></div><button type="button" className="rep-song-change" onClick={()=>chooseSetupRep(name)}>CHANGE ›</button></div>})}</div></>:<><button className="back-to-reps" onClick={()=>setSetupRep(null)}>‹ ALL SDRS</button><p>{setupSong?"Their saved song is loaded below. Paste a different URL to change it.":"The default song is loaded below. Paste a different URL to personalize it."}</p><div className="song-url-preview">{setupVideoId&&<img src={`https://i.ytimg.com/vi/${setupVideoId}/hqdefault.jpg`} alt="Selected YouTube song thumbnail"/>}<label>YOUTUBE LINK<input placeholder="https://youtube.com/watch?v=..." value={form.youtubeUrl} onChange={e=>setForm({...form,youtubeUrl:e.target.value})}/></label></div><label>START TIME IN SECONDS<input type="number" min="0" value={form.startSeconds} onChange={e=>setForm({...form,startSeconds:e.target.value})}/><small>Example: 1:12 into the song = 72 seconds. The site stops automatically 15 seconds later.</small></label>{formError&&<p className="form-error">{formError}</p>}<div className="modal-actions"><button onClick={()=>setSetupRep(null)}>BACK</button><button onClick={()=>setupRep&&testDemoComplete(setupRep)}>TEST DEMO COMPLETE</button><button className="test-button" onClick={saveSong}>{setupSong?"SAVE CHANGES":"SAVE PERSONAL SONG"}</button></div></>}</section></div>}
