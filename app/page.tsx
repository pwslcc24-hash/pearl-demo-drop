...
export default function Home() {
  ...
  const renderPodium=()=>(
    <div className="podium-stage"><div className="podium">{topThree.length?<><p className="podium-label">MONTHLY LEADERS · {monthLabel}</p>{topThree.map((entry,index)=><div key={entry.repName} className={`podium-row place-${(index+1) as 1|2|3}-row`}>{renderPodiumSlot(entry,(index+1) as 1|2|3)}</div>)}</>:<p className="podium-empty">Waiting for monthly counts from HubSpot…</p>}</div></div>
  );
