...
export default function Home() {
  ...
  const chooseSetupRep=(repName:string)=>{const saved=songs.find(s=>s.repName.toLowerCase()===repName.toLowerCase());setSetupRep(repName);setForm({repName,youtubeUrl:saved?`https://youtube.com/watch?v=${saved.videoId}`:"",startSeconds:String(saved?.startSeconds??0)});setFormError("")};
  ...
  const testDrop=()=>{const song=songs.find(s=>s.id===selectedSong)??songs[0];if(!song){openSetup();return}setArmed(true);const event={id:`${Date.now()}-${Math.random().toString(36).slice(2)}`,repName:song.repName,company:"Test Demo Office",product:"Demo completed",songId:song.id,createdAt:"Just now"};lastServerIdRef.current=event.id;launch(event,songs,true);fetch("/api/events",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(event)}).catch(()=>undefined)};
